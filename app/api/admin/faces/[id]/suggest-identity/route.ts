...
export async function POST(
  ...
  try {
    ...
    // Cap photoIds to 500 to keep the IN clause performant.
    const photoIds = personPhotos.map((pp) => pp.photoId).slice(0, 500);
...
Strong co-occurrence with a named person (especially a spouse, sibling, or repeated family events) is your primary signal.
...
{"suggestedName": "<full name, or null if cannot reasonably guess>", "reasoning": "<1–2 sentences explaining the inference>", "confidence": "high|medium|low"}`;
    ...
    console.error("[suggest-identity] error:", err);
