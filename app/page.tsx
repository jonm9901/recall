...
export default function SearchPage() {
  ...
  const search = useCallback(async (q: string, yr: string, tg: string, mr: string, pg: number) => {
    setLoading(true);
    try {
      ...
      <main className="flex-1 overflow-y-auto px-4 py-4">
        ...
        {pages > 1 && !loading && (
          <div className="flex items-center justify-center gap-2 mt-8 pb-4">
            <button
              onClick={() => { const p = page - 1; setPage(p); search(query, year, tag, minRating, p); }}
              disabled={page <= 1}
